    ...
    operator fun invoke(mouthDistances: MouthDistances): Triple<Boolean, Int, List<Long>> {
        ...
        if (mouthDistances.distanciaLabios > mouthDistances.distanciaMenton) {
            if (!isMouthOpen) {
                ...
                isMouthOpen = true
                Log.d(TAG, "👄 Boca abierta (Labios=${mouthDistances.distanciaLabios}, Mentón=${mouthDistances.distanciaMenton})")
            }
            ...
            //  MOSTRAR PROGRESO
            if (duration > 1000) {
                Log.d(TAG, "⏳ Bostezo en progreso: ${duration}ms / ${YAWN_DURATION_MS}ms")
            }
